...
export function FileDropZone({
  ...
  const validate = useCallback((fileList: FileList | File[]) => {
    const valid: File[] = []
    ...
    <div className="space-y-3">
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">PDF, JPG, PNG, DOCX — máximo 10MB por archivo</p>
        </div>
        <input ref={fileInputRef} type="file" multiple accept={allowedExtensions} onChange={handleSelect} className="hidden" />
      </div>
          ...
          {files.map(pf => (
            <div key={pf.id} className={cn(
              ...
              {pf.preview ? (
                ...
                <div className="flex h-10 w-10 items-center justify-center rounded bg-brand-navy/5">
                  {pf.file.type.startsWith("image/") ? (
                    <Image className="h-5 w-5 text-brand-sky" />
                  ) : (
              ...
              {pf.uploading ? (
                ...
                <button onClick={() => onRemoveFile(pf.id)} className="rounded p-1 text-muted-foreground hover:bg-muted hover:text-foreground">
                  <X className="h-4 w-4" />
                </button>
              )}
            </div>
